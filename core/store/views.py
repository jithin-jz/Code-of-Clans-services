...
class ImageUploadView(APIView):
    ...
    def post(self, request):
        ...
        path = default_storage.save(os.path.join(directory, file_obj.name), ContentFile(file_obj.read()))
...
class EquipItemView(APIView):
    ...
    def post(self, request):
        ...
        if not Purchase.objects.filter(user=user, item=item).exists():
             return Response({"error": "You do not own this item."}, status=status.HTTP_400_BAD_REQUEST)

        # Handle Themes
        if item.category == 'THEME':
            theme_key = item.item_data.get('theme_key')
            if not theme_key:
                 return Response({"error": "Invalid theme data."}, status=status.HTTP_400_BAD_REQUEST)
            
            user.profile.active_theme = theme_key
            user.profile.save()
            return Response({"status": "success", "message": f"Equipped {item.name}", "active_theme": theme_key})

        # Handle Fonts
        if item.category == 'FONT':
            font_family = item.item_data.get('font_family')
            if not font_family:
                return Response({"error": "Invalid font data."}, status=status.HTTP_400_BAD_REQUEST)
            user.profile.active_font = font_family
            user.profile.save()
            return Response({"status": "success", "message": f"Equipped {item.name}", "active_font": font_family})
        ...
        if item.category == 'EFFECT':
            ...
            user.profile.save()
            return Response({"status": "success", "message": f"Equipped {item.name}", "active_effect": effect_key})
        ...
        if item.category == 'VICTORY':
            ...
            user.profile.save()
            return Response({"status": "success", "message": f"Equipped {item.name}", "active_victory": victory_key})

        return Response({"error": "This item cannot be equipped."}, status=status.HTTP_400_BAD_REQUEST)
...
class UnequipItemView(APIView):
    ...
    def post(self, request):
        ...
        user.profile.save()
        return Response({"status": "success", "message": f"Unequipped {category}"})
