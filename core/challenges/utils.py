...
def generate_certificate_image(user_certificate):
    ...
    # Inner border
    draw.rectangle(
        [(border_width + 20, border_width + 20), (width - border_width - 20, height - border_width - 20)],
    ...
    # 4. Draw Content
    # Title
    draw.text((width/2, 200), "CERTIFICATE OF COMPLETION", fill="black", font=title_font, anchor="mm")

    # Body
    draw.text((width/2, 400), "This certifies that", fill="gray", font=text_font, anchor="mm")
    
    # User Name
    user_name = user_certificate.user.username.upper() # Use Full Name if available in profile
    draw.text((width/2, 550), user_name, fill=border_color, font=name_font, anchor="mm")

    # Course Info
    draw.text((width/2, 700), "Has successfully completed the", fill="black", font=text_font, anchor="mm")
    draw.text((width/2, 800), "PYTHON MASTERY COURSE", fill="black", font=name_font, anchor="mm")
    
    # Date
    date_str = user_certificate.issued_at.strftime("%B %d, %Y")
    draw.text((width/2, 900), f"Issued on: {date_str}", fill="gray", font=text_font, anchor="mm")
    ...
    # Platform Name
    draw.text((width - 400, 1100), "Code of Clans", fill="black", font=name_font, anchor="mm")
